...
async def search_debates(
    *,
    ...
    # Group by debate and deduplicate
    debates_map = {}
    for result in search_results:
        ...
        if debate_id and debate_id not in debates_map:
            debates_map[debate_id] = {
                "debate_id": debate_id,
                "title": payload.get("DebateSection", ""),
    ...
    # Return top results
    debates = list(debates_map.values())
    if query:
        # Sort by relevance score if we have a query
        debates.sort(key=lambda x: x["relevance_score"], reverse=True)

    return debates[:max_results]
